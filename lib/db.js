...
const GYMS_FILE = path.join(DB_DIR, 'venues.json');
...
export async function getAllGyms(filters = {}) {
  let venues = await readJSON(GYMS_FILE);
  if (filters.city) venues = venues.filter(g => g.city === filters.city);
  if (filters.crowd) venues = venues.filter(g => g.crowdLevel === filters.crowd);
  if (filters.maxPrice) venues = venues.filter(g => g.pricePerHour <= Number(filters.maxPrice));
  if (filters.search) {
    const s = filters.search.toLowerCase();
    venues = venues.filter(g => g.name.toLowerCase().includes(s) || g.address.toLowerCase().includes(s));
  }
  return venues.sort((a, b) => b.rating - a.rating);
}

export async function getGymById(id) {
  const venues = await readJSON(GYMS_FILE);
  return venues.find(g => g._id === id) || null;
}
